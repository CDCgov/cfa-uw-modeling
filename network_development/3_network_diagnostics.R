...
## note: for some reason, the plots look worse when using
# SA than using MCMLE, but other metrics ok
## note: GOF metrics will be slightly different every time it is run,
...
## Note: Because the ERGM terms used as target statistics in the model
# fit do not discretely represent each
## age and race degree combination, the degree distributions will not
#match the empirical distributions perfectly,
## but should be similar in shape.
##
## Note 2: The networks within the netest and netdx objects are independent
# of each other, and the netdx
## objects are simulated in a closed population. Thus, we will need to run a
# netsim simulation to see how
